...
# def exclude_outliers(data, group_idx):
...
#         group_lbl = []
#         for i, flag in enumerate(outlier_flag):
#             if flag:
#                 group_lbl.append("Young" if i in group_idx[0] else "Old")
#         print(f"\t[State/Mode {n}] # outliers: {n_outliers} subjects ({group_lbl.count('Young')} young, {group_lbl.count('Old')} old)")

#         typical_flag = np.invert(outlier_flag)
#         data_clean.append(features[typical_flag])
#     return None
