...
class GroupPSDDifference():
    ...
    def plot_diff_alpha(self, clusters, save_dir):
        # Compute frequencies to draw topomaps
        alpha_range = np.where(np.logical_and(self.freqs >= 8, self.freqs <= 12))
        gpsd_diff_alpha = self.gpsd_diff_avg[alpha_range]
        low_peak = self.freqs[self.gpsd_diff_avg == min(gpsd_diff_alpha)]
        high_peak = self.freqs[self.gpsd_diff_avg == max(gpsd_diff_alpha)]
        topo_freq = [high_peak, low_peak]
        
        # Get maximum and minimum values for topomaps    
        if self.data_space == "sensor" and self.modality == "meg":
            vmin, vmax = self._get_minmax(self.gpsd_diff_mag[:, alpha_range])
        else:
            vmin, vmax = self._get_minmax(self.gpsd_diff[:, alpha_range])
        ...
        if self.data_space == "source":
            ...
            fig, ax = plotting.plot_psd_topo(
                ...
                topomap_pos=[0.45, 0.35, 0.5, 0.55],
            )
            # Shrink axes to make space for topos
            fig.set_size_inches(7, 8)
            ax_pos = ax.get_position()
            ax.set_position([ax_pos.x0, ax_pos.y0, ax_pos.width, ax_pos.height * 0.75])
            ...
            # Get topo centers
            topo_centers = np.linspace(0, 1, len(topo_freq) + 2)[1:-1]
            # Plot topomaps
            topo_type = "surface"
            for i in range(len(topo_freq)):
                ...
                if topo_type == "marker":
                    ...
                    vmin = -vmax # nilearn.plot_glass_brain() internally uses -vmax as vmin
                ...
                ax.add_artist(con)
                ax.axvline(x=topo_freq[i], color='tab:gray', lw=2)
            ...
            # Plot topomaps
            if self.modality == "eeg":
                topo_raw = self.raw.pick_types(eeg=True, meg=False).reorder_channels(ch_names)
                for i in range(len(topo_freq)):
                    gpsd_data = np.squeeze(np.array(self.gpsd_diff[:, self.freqs == topo_freq[i]]))
                    topo_pos = [topo_centers[i] - 0.2, 1.1, 0.25, 0.25]
                    topo_ax = ax.inset_axes(topo_pos)
                ...
                ch_names = np.array([ch['ch_name'] for ch in chs])
                topo_raw = self.raw.pick_types(eeg=False, meg='mag').reorder_channels(ch_names)
                for i in range(len(topo_freq)):
        ...
        # Save figure
        save_path = os.path.join(save_dir, "psd_diff_alpha.png")
    ...
    def plot_diff_low_beta(self, clusters, save_dir):
        # Compute frequencies and inputs to draw topomap
        freq_range = [[1, 8], [13, 30]]
        topo_data = [
            analysis.power.variance_from_spectra(self.freqs, self.gpsd_diff, frequency_range=freq_range[0]),
            analysis.power.variance_from_spectra(self.freqs, self.gpsd_diff, frequency_range=freq_range[1]),
        ...
        if self.data_space == "sensor" and self.modality == "meg":
            topo_data_mag = [
                analysis.power.variance_from_spectra(self.freqs, self.gpsd_diff_mag, frequency_range=freq_range[0]),
                analysis.power.variance_from_spectra(self.freqs, self.gpsd_diff_mag, frequency_range=freq_range[1]),
            ]
            vmin, vmax = np.min(topo_data_mag), np.max(topo_data_mag)
        else:
            vmin, vmax = np.min(topo_data), np.max(topo_data)
        ...
        if self.data_space == "source":
            ...
            for i in range(len(topo_freq)):
                ...
                con = matplotlib.patches.ConnectionPatch(xyA=xy, xyB=(np.mean(topo_ax.get_xlim()), topo_ax.get_ylim()[0]),
                                                        ...
                                                        axesA=ax, axesB=topo_ax, color='tab:gray', lw=2)
                ax.add_artist(con)
                ax.axvspan(freq_range[i][0], freq_range[i][1], facecolor='k', alpha=0.1, zorder=0)
        elif self.data_space == "sensor":
            # Start a figure object
            fig, ax = plt.subplots(nrows=1, ncols=1)
            cmap = plt.cm.viridis_r
        ...
        if self.data_space == "source":
            ...
            for i in range(len(topo_freq_top)):
                ...
                if topo_type == "marker":
                    ...
                    vmax_top = np.max(np.abs([vmin_top, vmax_top]))
                    plot_surfaces(
                        gpsd_data,
                        self.mask_file,
                        self.parcellation_file,
                    ...
                    vmin_top = -vmax_top # nilearn.plot_glass_brain() internally uses -vmax as vmin
                # Connect frequencies to topographical map
            ...
            for i in range(len(topo_freq_bottom)):
                ...
                if topo_type == "marker":
                    ...
                    vmax_bottom = np.max(np.abs([vmin_bottom, vmax_bottom]))
                    plot_surfaces(
                        topo_data[i],
                        self.mask_file,
                        self.parcellation_file,
                    ...
                    vmin_bottom = -vmax_bottom # nilearn.plot_glass_brain() internally uses -vmax as vmin
                # Connect frequencies to topographical map
